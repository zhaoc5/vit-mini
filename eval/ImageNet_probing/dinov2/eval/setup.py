...
def build_model_for_eval_proteus(config, pretrained_weights):
    model, _ = build_model_from_cfg(config, only_teacher=True)
    dinov2_utils.load_pretrained_weights_proteus(model, pretrained_weights, "teacher")
    model.eval()
    model.cuda()
    return model
...
def setup_and_build_model_proteus(args) -> Tuple[Any, torch.dtype]:
    cudnn.benchmark = True
    config = setup(args)
    model = build_model_for_eval_proteus(config, args.pretrained_weights)
    autocast_dtype = get_autocast_dtype(config)
    return model, autocast_dtype